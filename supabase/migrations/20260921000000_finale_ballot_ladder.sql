...
--
-- Each correct name is now worth double the one before it, and the winner is the
-- last double:
--
--   Final 4    2 / 4 / 8 / 16   (cumulative  2 ·  6 · 14 · 30)
--   Final 3    5 / 10 / 20      (cumulative  5 · 15 · 35)
--   Winner     40               (unchanged — the next double after 20)
--
-- Par drops to 29 of 65; a perfect bracket with the winner goes 100 -> 105. The
-- Final 3 ladder restarts at 5 rather than continuing from 16 because the slates
-- nest: shifted by a single rung, the increments coincide and "all four right,
-- wrong fire-making boot" would tie "missed one of the four but nailed the
-- Final 3". Restarting keeps the endgame read ahead, 49 to 45.
--
-- perfect_final_three is retired — it was this curve bolted onto a flat rate,
-- and the ladder builds it into the 20-point top rung.
--
...
values
  ...
  ('correct_final_three_1', '1st correct Final 3 name',   5, null),
  ('correct_final_three_2', '2nd correct Final 3 name',  10, null),
  ('correct_final_three_3', '3rd correct Final 3 name',  20, null)
...
-- S27 and S270 are the prod practice seasons (one human, a field of bots); S51
-- is the real league's season, not yet started. S270's finale is already scored
-- (#881) and nothing caches a score, so it rescores at these values on the next
-- read — which is the point, it is the only real data the curve can be checked
-- against. Every other season keeps its snapshot exactly as it is.
