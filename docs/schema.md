...
| Episode facts | `eliminations`, `scoring_events` | Commissioner-reviewed facts about what happened. Placements on `contestants` synchronize finale placement events through database triggers. `eliminations.is_final` is false for a holding-pen boot — Redemption Island (#655) or Edge of Extinction: the ballot scores the row, but "out of the game" everywhere else means an elimination with `is_final`. On Redemption the terminal exit is a `redemption_loss` (a lost duel); the Edge has no duels, so its non-returnees stay non-final until the final return challenge, then the commissioner flips them. `tribes.is_redemption` marks the holding-pen tribe the survivoR sync creates (`tribe_import.HOLDING_PEN_TRIBES`), whose residents are hidden from the ballot. |
